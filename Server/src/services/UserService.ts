...
export async function register(user: IUser): Promise<IUserModel> {
  ...
  try {
    ...
    const created = await UserDao.insert({
      ...user,
      type: 'PATRON',
...
export async function login(credentials: { email: string; password: string }): Promise<IUserModel> {
  ...
  try {
    ...
    if (user.status === 'PENDING')
      throw new AccountPendingApprovalError('Your account is awaiting admin approval');
...
export async function promoteUser(userId: string): Promise<IUserModel> {
  ...
  if (target.type === 'ADMIN')
    throw new InvalidRoleTransitionError("The admin's role cannot be changed");
  if (target.type === 'EMPLOYEE')
    throw new InvalidRoleTransitionError('User is already an employee');
