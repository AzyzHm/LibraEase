...
| `environment.production.ts` | generated at build time from `API_BASE_URL` | `npm run build:prod` |

`environment.production.ts` is not checked in as a hand-edited file, it's generated by `scripts/generate-env.js` every time `build:prod` runs (wired up as a `prebuild:prod` npm hook). That script reads `API_BASE_URL` from the environment and writes it into the file, it never edits the file itself, so don't hand-edit `environment.production.ts`, it'll be overwritten on the next build. In CI or on Vercel (detected via the `CI`/`VERCEL` env vars) the script fails the build loudly if `API_BASE_URL` isn't set, in plain local builds it falls back to `http://localhost:8000` with a warning. On the live Vercel deployment, `API_BASE_URL` itself is kept in sync automatically by the CD pipeline (see [`cd.yml`](../.github/workflows/cd.yml)), not set by hand in the Vercel dashboard.
...
`AuthStore` is the one exception worth knowing about: it holds no persisted client-side state at all, session survives a page refresh because the backend session cookie does, not because the frontend saved anything. On construction, it calls `/auth/me` to restore the user object into its signal (see [Authentication](#authentication)).
